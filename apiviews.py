...
class CarsNeedingServicing(generics.ListAPIView):
    def get_queryset(self):
        ...
        queryset = AllCarsNeedingServicing.objects.filter(current_millage__gte = 20000)
        return queryset
    serializer_class = AllCarsNeedingServicingSeializer #CarSerializer


class  CheckCars(generics.ListAPIView):
    def get_queryset(self):
        allcars = Car.objects.all() #select_related('car_type').filter(car_type__engine__name="Capulet Engine")
        allcars_for_service = [car for car in allcars if car.current_millage >= 20000]
        # for car in allcars_for_service:
        #     queryset = car
            #engine = item.
        ...
        return allcars_for_service
    serializer_class = AllCarsNeedingServicingSeializer
